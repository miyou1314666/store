...
def start_job(files: dict[str, tuple[str, bytes]], fields: dict[str, str]) -> str:
    job_id = uuid.uuid4().hex
    with JOBS_LOCK:
        JOBS[job_id] = {"status": "processing", "message": "文件已上传，正在处理 Excel。"}

    def run() -> None:
        try:
            result = process_payload(files, fields)
            with JOBS_LOCK:
...
def get_job(job_id: str) -> dict[str, Any]:
    with JOBS_LOCK:
        job = JOBS.get(job_id)
    if not job:
        raise ValueError("未找到处理任务，请重新上传文件。")
    return job
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if request_path.startswith("/api/status/"):
            job_id = request_path.rsplit("/", 1)[-1]
            try:
                self.send_json(200, get_job(job_id))
            except Exception as exc:
                self.send_json(404, {"status": "error", "error": str(exc)})
            return
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if "sapFile" not in files or "srmFile" not in files:
                raise ValueError("请同时上传 SAP 前三个月用量表和 SRM 未来预测表。")
            job_id = start_job(files, fields)
            self.send_json(202, {"status": "processing", "jobId": job_id})
        except Exception as exc:
